...
const RICH: Record<number, RichContent> = {
  1: {
    ...
    decisionFramework: {
      checkpoints: [
        '**时位**：你现在处于哪一爻？潜（隐藏期）/ 见（显现期）/ 惕（试探期）/ 跃（突破期）/ 飞（当道期）/ 亢（过盛期）',
        '**应位**：是否有合适的"应"——共事者、机会、外部条件？乾卦无内外阴阳之配，所以特别强调"独立判断、不依赖呼应"。',
        '**变动方向**：进、退、守，哪个最配当下时位？亢龙的悔不是因为飞错了，是因为没在该退时退。',
      ],
      scenario:
        '乾卦六爻像极了一段创业旅程：潜龙是 idea 还在酝酿、不要急着对外宣传；见龙是 MVP 出来开始让"大人"（用户、投资人、伙伴）看见；惕龙是 PMF 验证期，每天战战兢兢；跃龙是 scaling 前的最后试探；飞龙是行业领导期；亢龙——是大多数曾经的飞龙没读懂的那一爻：当你已经飞在天上，你最该做的是为下一次潜龙做准备，而不是再飞高一点。',
    },
  },
  2: {
